...
function renderScheduleUI(games) {
  ...
  // ⚡ Bolt: Calculate counts in a single O(N) pass to prevent redundant array traversals
  // Impact: Replaces 3x O(N) .filter() calls that created discarded intermediate arrays, reducing garbage collection overhead during render.
  let liveCount = 0;
  let upcomingCount = 0;
  let finishedCount = 0;
