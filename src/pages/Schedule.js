...
async function loadSchedule() {
  ...
  try {
    ...
    // Pre-compute search strings to avoid repeated object traversal and
    // .toLowerCase() calls on every keystroke during filtering.
...
function renderScheduleUI(games) {
  ...
  // Single-pass count avoids allocating 3 intermediate arrays
  let liveCount = 0;
  let upcomingCount = 0;
  let finishedCount = 0;
...
function applyFilters() {
  ...
  // Single-pass: combine status and search filters.
  // Uses pre-computed _searchString to avoid repeated object traversal
  // and .toLowerCase() calls on every keystroke.
  const filtered = currentGames.filter(game => {
    if (filterValue !== 'all' && game.status !== filterValue) return false;
    if (query && !game._searchString.includes(query)) return false;
    return true;
  });
