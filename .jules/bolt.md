...
## 2024-05-19 - Intl Formatter Initialization Overhead vs Localization
**Learning:** `toLocaleString()` and `toLocaleDateString()` incur significant overhead inside tight loops or repeated component renders because they instantiate a new `Intl.NumberFormat` or `Intl.DateTimeFormat` object on every call. However, when caching these instances to improve performance (e.g., `new Intl.NumberFormat('en-US')`), it's crucial to use `undefined` as the locale (e.g., `new Intl.NumberFormat(undefined)`) if you want to preserve the default system locale settings and avoid localization regressions.
**Action:** When optimizing repeated format calls, cache the `Intl` instance globally at the module level. Always initialize the locale parameter to `undefined` (or omit it) unless there is an explicit requirement to force a specific locale.
