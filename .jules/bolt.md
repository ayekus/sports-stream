...
## 2026-03-10 - [Inefficient Multiple Array Filters]\n**Learning:** Multiple sequential `.filter()` array methods chained together on the same data set to calculate subsets lengths cause severe garbage collection overhead and repeated O(N) traversals.\n**Action:** Avoid chaining or sequentially calling multiple `.filter()` array methods to generate counts, lengths, or categorize elements. Instead, use a single-pass `for...of` loop or `.reduce()`.
