...
## 2026-03-10 - [Chained and Sequential Filter Operations]
**Learning:** In `src/pages/Schedule.js`, the code was making extensive use of chained and sequential `.filter()` operations to calculate lengths and apply different sets of search and filter constraints. This caused multiple unnecessary traversals through the data and allocated excessive intermediate arrays, putting pressure on garbage collection.
**Action:** Always attempt to combine sequential or chained `.filter()` calls into a single loop or array pass. When computing counts/lengths based on categories, use a single `for...of` loop or `.reduce()` to execute in O(N) time without allocating garbage intermediary arrays.