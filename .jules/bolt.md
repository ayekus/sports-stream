...
## 2026-06-25 - [Redundant Filter Passes in Standings View]
**Learning:** `src/pages/Standings.js` frequently parsed the `standings` array by calling `.filter(t => t.conferenceAbbrev === 'E')` and then again for `'W'` (in `getPlayoffStatus`, `renderWildCardView`, and `renderConferenceView`). Executing multiple consecutive `.filter()` passes on the same array to categorize its elements results in redundant O(N) array traversals and unnecessary intermediate array creation, which degrades UI responsiveness during frequent recalculations or view switching.
**Action:** Replace multiple sequential `.filter()` operations that categorize elements based on a property with a single-pass traversal (e.g., using a `for...of` loop or `.reduce()`) that pushes items into their respective category arrays simultaneously. This optimizes the time complexity from O(K*N) to O(N) where K is the number of filters.
