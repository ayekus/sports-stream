...
## 2026-03-11 - [Inefficient Date and Number Formatting in Render Loops]
**Learning:** Functions like `toLocaleString()` and `toLocaleDateString()` are computationally expensive because they instantiate a new `Intl.DateTimeFormat` or `Intl.NumberFormat` internally on each call. When used inside render methods or frequently called functions (e.g., updating UI components, formatting salaries, or tracking a countdown), this causes noticeable CPU overhead and memory allocation pressure.
**Action:** Always create a cached `Intl.DateTimeFormat` or `Intl.NumberFormat` instance outside of loops or at the module level and reuse its `.format()` method. For example, export cached formatters in utility modules (like `src/utils/date.js`) and use them in components to avoid repeated initializations.
